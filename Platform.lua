...
print("📦 [BRAIN] Loading Platform module...")
local Platform = loadstring(game:HttpGet("https://raw.githubusercontent.com/ZebuxHub/Realmain/refs/heads/main/Platform.lua"))()
print("✅ [BRAIN] Platform module loaded!")
...
local Settings = {
    ...
    -- Auto Unlock Platform
    AutoUnlockPlatform = false,
    -- Auto Equip Brainrots
...
local function SaveConfig(configName)
    local success, err = pcall(function()
        local configData = {
            ...
            AutoUnlockPlatform = Settings.AutoUnlockPlatform,
            AutoEquipBrainrots = Settings.AutoEquipBrainrots,
            EquipInterval = Settings.EquipInterval
        }
...
local function LoadConfig(configName)
    ...
    local success, err = pcall(function()
        ...
        Settings.AutoUnlockPlatform = configData.AutoUnlockPlatform or false
        Settings.AutoEquipBrainrots = configData.AutoEquipBrainrots or false
        Settings.EquipInterval = configData.EquipInterval or 5
...
local platformReferences = {
    LocalPlayer = LocalPlayer
}
Platform.Init(services, platformReferences, Brain)
Brain.Platform = Platform
...
--[[
    ========================================
    Step 12: Platform Tab
    ========================================
--]]

local platformTab = section:Tab({
    Title = "Platform",
    Icon = cascade.Symbols.squareStack3dForwardDottedline,
})

local platformForm = platformTab:PageSection({ 
    Title = "Auto Unlock Platform",
    Subtitle = "Automatically unlock platforms when you have enough money & rebirth"
}):Form()

-- Auto Unlock Platform Toggle
local platformRow = platformForm:Row()

platformRow:Left():TitleStack({
    Title = "Auto Unlock",
    Subtitle = "Enable to auto-unlock platforms",
})

platformRow:Right():Toggle({
    Value = Settings.AutoUnlockPlatform or false,
    ValueChanged = function(self, value)
        Settings.AutoUnlockPlatform = value
        Platform.Settings.AutoUnlockEnabled = value
        
        print("⚙️ [BRAIN] Auto Unlock Platform: " .. (value and "ENABLED" or "DISABLED"))
        
        if value then
            -- Start platform unlocking
            if not Platform.IsRunning then
                Platform.Start()
            end
        else
            -- Stop platform unlocking
...
-- Auto Equip Best Brainrots Section
local equipForm = platformTab:PageSection({ 
    Title = "Auto Equip Best Brainrots",
    Subtitle = "Automatically equip your best brainrots at a set interval"
}):Form()

-- Auto Equip Toggle
local equipRow = equipForm:Row()

equipRow:Left():TitleStack({
    Title = "Auto Equip",
    Subtitle = "Enable to auto-equip best brainrots",
})

equipRow:Right():Toggle({
    Value = Settings.AutoEquipBrainrots or false,
    ValueChanged = function(self, value)
        Settings.AutoEquipBrainrots = value
        Platform.Settings.AutoEquipBrainrotsEnabled = value
        
        print("⚙️ [BRAIN] Auto Equip Best Brainrots: " .. (value and "ENABLED" or "DISABLED"))
        
        if value then
            -- Start auto-equip loop
            Platform.StartEquipLoop()
        else
            -- Stop auto-equip loop
            Platform.StopEquipLoop()
        end
    end,
})

-- Equip Interval Stepper
local intervalRow = equipForm:Row()

intervalRow:Left():TitleStack({
    Title = "Equip Interval",
    Subtitle = "Minutes between auto-equip",
})

intervalRow:Right():Stepper({
    Value = Settings.EquipInterval or 5,
    Minimum = 1,
    Maximum = 60,
    Step = 1,
    ValueChanged = function(self, value)
        Settings.EquipInterval = value
        Platform.Settings.EquipInterval = value
        
        print("⚙️ [BRAIN] Equip Interval: " .. value .. " minutes")
        
        -- Restart equip loop if running to apply new interval
...
-- Manual Equip Button
local manualRow = equipForm:Row()

manualRow:Left():TitleStack({
    Title = "Equip Now",
    Subtitle = "Manually equip best brainrots",
})

manualRow:Right():Button({
    Label = "Equip",
    State = "Primary",
    Pushed = function(self)
        print("🔄 [BRAIN] Manually equipping best brainrots...")
