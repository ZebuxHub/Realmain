...
local platformStatusForm = platformTab:PageSection({ 
    Title = "Platform Status",
    Subtitle = "View all platforms and requirements"
}):Form()

-- Refresh button to show current platforms
local refreshRow = platformStatusForm:Row()
refreshRow:Left():TitleStack({
    Title = "Platform List",
    Subtitle = "Click refresh to see all platforms",
})

refreshRow:Right():Button({
    Label = "Refresh",
    State = "Primary",
    Pushed = function(self)
        print("🔄 [BRAIN] Refreshing platform list...")
        ...
        print("=====================================")
        print("🏗️ Platform Status Report")
        print("=====================================")
        print("💰 Money: $" .. FormatNumber(currentMoney))
        print("🔄 Rebirth: " .. currentRebirth)
        print("-------------------------------------")
        ...
        if #platforms == 0 then
            ...
            for _, platform in ipairs(platforms) do
                local status = platform.Enabled and "✅ UNLOCKED" or "🔒 LOCKED"
                local canAfford = currentMoney >= platform.Price and "💰" or "❌"
                local canRebirth = currentRebirth >= platform.RebirthRequired and "✅" or "❌"
                
                print(string.format(
                    "#%s | %s | Price: $%s %s | Rebirth: %d %s",
                    platform.Number,
                    status,
                    FormatNumber(platform.Price),
                    canAfford,
                    platform.RebirthRequired,
                    canRebirth
                ))
            end
        end
